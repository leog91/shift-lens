...
export function BatchExtractionButton({ weekId, dailySheetCount, unprocessedDocuments, reviewOnlyDocuments }: { weekId: string; dailySheetCount: number; unprocessedDocuments: number; reviewOnlyDocuments: number }) {
  ...
  async function runBatchExtraction() {
    ...
    try {
      ...
      setMessage(`Processed ${result.processedDocuments ?? 0} paper(s), created ${createdRows} needs-checking row(s)${failedDocuments.length ? `, ${failedDocuments.length} failed` : ""}.`);
      if (createdRows > 0) window.setTimeout(() => window.location.reload(), 1800);
    } catch (error) {
  ...
  return (
    <div className="rounded border bg-white p-4">
      ...
      <button className="mt-2 rounded bg-blue-700 px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50" disabled={running || dailySheetCount === 0} onClick={runBatchExtraction} type="button">{running ? "Analyzing papers..." : "Rescan papers and add missing OCR rows"}</button>
