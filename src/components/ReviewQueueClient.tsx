...
export function ReviewQueueClient({ reviewItems }: { reviewItems: ReviewItemView[] }) {
          ...
          <article className="grid gap-4 rounded-lg border bg-white p-4 md:grid-cols-[360px_1fr]" key={item.id}>
            <div className="space-y-2">
              <div className="overflow-hidden rounded border bg-stone-100">
                {item.documentPath ? <img alt={`Source document ${item.filename}`} className="max-h-[420px] w-full object-contain" src={`/api/local-photo?path=${encodeURIComponent(item.documentPath)}`} /> : <div className="flex h-36 items-center justify-center text-stone-500">Source image unavailable</div>}
              </div>
              {item.documentPath ? <a className="text-sm" href={`/api/local-photo?path=${encodeURIComponent(item.documentPath)}`} target="_blank">Open full document</a> : null}
            </div>
            <div className="space-y-2">
              ...
              <input className="w-full max-w-md rounded border p-2" onChange={(event) => setValues((current) => ({ ...current, [item.id]: event.target.value }))} placeholder={placeholder} value={values[item.id] ?? ""} />
              {errors[item.id] ? <p className="text-sm text-red-700">{errors[item.id]}</p> : null}
              <div className="flex flex-wrap gap-2">
                <button className="rounded bg-blue-700 px-3 py-2 text-white disabled:opacity-50" disabled={isPending} onClick={() => confirm(item)} type="button">Save confirmed value</button>
              </div>
            </div>
          </article>
        );
